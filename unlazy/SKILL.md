...
**Orchestrated**. The task is a build: tree depth 4 or more, or clearly beyond one sitting. Decompose per [references/method.md](references/method.md), write `PLAN.md` plus one gates file per leaf under `gates/`, and run each leaf as a fresh subagent with a narrow brief. Read [references/orchestration.md](references/orchestration.md) before fanning out. Two things there are the point of the mode: the verification hierarchy (leaf checks itself, parent re-runs the checks), and **wave dispatch**: spawn every leaf whose blockers are verified *at the same time*, up to the harness cap, never one at a time. The contract's disjoint file ownership is what makes that safe.
